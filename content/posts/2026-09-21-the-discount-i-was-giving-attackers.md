...
The fix is not a switch. Every one of the 75 released injections has the classifier as its only finding — the dampener fires *only* when no structural rule hit, so anything the structural layer catches is immune to it by the feature's own precondition. And those 75 are not exotic. They fall into four families: exfiltration demands, coercion framing, prefix seeding through a creative-writing request, and plain rule overrides like *forget about the rules and policies set forth by the system administrator*, which our rules miss only because they were written to catch "ignore previous instructions".

Covering those families structurally closes the bypass without touching the dampener at all. That is the next piece of work, and it ships with the same harness attached, against all four benign sets, because the last time we improved detection by intuition it cost 23 points of recall and every test still passed.
